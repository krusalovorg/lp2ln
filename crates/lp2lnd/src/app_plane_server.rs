...
async fn handle_connection(
    ...
    loop {
        ...
        let cmd = match decode_cmd(&body) {
            Ok(c) => c,
            Err(_) => {
                let ack = encode_event(&AppEvent::Ack { ok: false, error: Some("bad frame".into()) });
        ...
        match cmd {
            AppCmd::Subscribe { protocol_id } => {
                ...
                let ack = encode_event(&AppEvent::Ack { ok: true, error: None });
                if stream.write_all(&ack).await.is_err() { break; }
            }
            ...
            AppCmd::Send { peer_id, protocol_id, payload } => {
                let result = send_app_packet(&node, &my_peer_id, &peer_id, protocol_id, payload).await;
                let ack = match result {
                    Ok(_) => encode_event(&AppEvent::Ack { ok: true, error: None }),
                    Err(e) => encode_event(&AppEvent::Ack { ok: false, error: Some(e.to_string()) }),
                };
                if stream.write_all(&ack).await.is_err() { break; }
            }
        }
...
async fn send_app_packet(
    ...
    let router = node.router().ok_or_else(|| anyhow::anyhow!("node not started"))?;
    let pkt = Packet {
        signature: None,
        data: payload,
        nodes: vec![],
    ...
    router.send_to_peer(PeerId::from(peer_id), pkt, None).await?;
    Ok(())
}
