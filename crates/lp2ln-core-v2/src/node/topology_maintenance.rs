...
use crate::node::distribution::{
    ...
    rank_dial_candidates, regular_auto_dial_target, should_reseed_bootstrap, EXPLORATORY_SLOTS,
    EXPLORATORY_SLOTS_MAX,
    should_skip_for_bootstrap_quota, stable_bootstrap_reseed_guard, DIAL_HUB_SOFT_CAP_EXTRA,
    REGULAR_SELF_HEAL_FLOOR,
};
use crate::node::options::{
    AdaptiveTopologyProfile, BootstrapNode, NodeOptions, NodeRole, TopologyTuning,
};
use crate::packet::Packet;
use crate::peer_score::{total_score, PeerConnectionPolicy, PeerScore, PeerScoreStore, PeerScoreWeights};
...
fn adaptive_tick_policy(
    ...
    if tuning.adaptive_topology_enabled {
        dial_target = (((base_dial_target as f32) * target_mul).round() as usize)
            .clamp(tuning.adaptive_target_min_floor, tuning.adaptive_target_max_ceil);
...
fn rank_bootstrap_targets_for_peer(local_peer_id: &str, targets: &[BootstrapNode]) -> Vec<BootstrapNode> {
        ...
        loop {
            ...
            if n < dial_target_low || should_explore {
                ...
                for pid in candidates {
                    ...
                    let max_endpoints =
                        dial_endpoint_attempt_budget(n, dial_target, adaptive.min_active_peers, addr_count);
            ...
            if now.saturating_sub(last_publish) >= descriptor_interval.as_millis() as u64 {
                ...
                let effective_cap =
                    (((base_cap as f32) * pressure_factor).round() as u16).max(4);
