...
use crate::services::NatTraversalPort;
...
impl NatTraversalPort for NatTraversalState {
    fn cleanup_expired(&self) {
        NatTraversalState::cleanup_expired(self);
    }
    ...
    fn create_offer(
        ...
        NatTraversalState::create_offer(self, session_id, peer_id, local_candidates)
    }
    ...
    fn handle_offer(
        ...
        NatTraversalState::handle_offer(self, from, offer, local_candidates)
    }

    fn handle_answer(&self, from: &str, answer: &NatAnswerPayload) -> Option<u64> {
        NatTraversalState::handle_answer(self, from, answer)
    }

    fn mark_punch_start(&self, session_id: &str, start_after_ms: u64) {
        NatTraversalState::mark_punch_start(self, session_id, start_after_ms);
    }

    fn mark_punching(&self, session_id: &str) {
        NatTraversalState::mark_punching(self, session_id);
    }
    ...
    fn mark_result(
        ...
        NatTraversalState::mark_result(self, session_id, success, selected_addr, reason);
    }
}
...
mod tests {
    ...
    fn state_machine_offer_answer_flow() {
        ...
        let offer = state.create_offer("sess-a".to_string(), "peer-b".to_string(), local.clone());
