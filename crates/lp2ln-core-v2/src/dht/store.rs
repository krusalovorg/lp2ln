...
impl DhtStore {
    ...
    pub fn load_from_db(&self, db: &P2PDatabase) {
        ...
        let Ok(table) = txn.open_table(crate::db::tables::DHT_TABLE) else { return };
            ...
            if let Ok(e) = postcard::from_bytes::<DhtStoreEntry>(v.value()) {
                match e {
                    ...
                    DhtStoreEntry::Providers(list) => {
                        let live: Vec<_> = list.into_iter().filter(|p| !p.provider.is_expired()).collect();
                        if !live.is_empty() { self.providers.insert(key, live); }
                    }
                    _ => {}
                }
            }
    ...
    fn persist_node(db: &P2PDatabase, key: &str, record: &NodeRecord) {
        let Ok(bytes) = postcard::to_allocvec(&DhtStoreEntry::Node(record.clone())) else { return };
        let lock = db.db.lock().expect("db lock");
    ...
    fn persist_providers(db: &P2PDatabase, key: &str, list: &[ProviderRecord]) {
        let Ok(bytes) = postcard::to_allocvec(&DhtStoreEntry::Providers(list.to_vec())) else { return };
        let lock = db.db.lock().expect("db lock");
        let Ok(txn) = lock.begin_write() else { return };
        if let Ok(mut t) = txn.open_table(crate::db::tables::DHT_TABLE) {
            t.insert(key, bytes.as_slice()).ok();
    ...
    pub fn put_node(&self, record: NodeRecord, db: Option<&P2PDatabase>) -> bool {
        if record.is_expired() { return false; }
        let key = hex::encode(record.node_id);
        let accepted = {
            match self.nodes.get(&key) {
                Some(existing) if existing.seq > record.seq => false,
                _ => { self.nodes.insert(key.clone(), record.clone()); true }
            }
        };
        if accepted {
            if let Some(db) = db { Self::persist_node(db, &key, &record); }
        }
        accepted
    }
    ...
    pub fn put_provider(&self, record: ProviderRecord, db: Option<&P2PDatabase>) {
        ...
        if let Some(pos) = entry.iter().position(|p| p.provider.peer_id == record.provider.peer_id) {
            entry[pos] = record.clone();
        } else {
            if entry.len() < 20 { entry.push(record.clone()); }
        }
        if let Some(db) = db { Self::persist_providers(db, &key, &entry); }
    }
    ...
    pub fn get_providers(&self, content_id: &[u8; 32]) -> Vec<ProviderRecord> {
        ...
        self.providers
            .get(&key)
            .map(|v| v.iter().filter(|p| !p.provider.is_expired()).cloned().collect())
            .unwrap_or_default()
    }
